...
/*
kvdoel@cs.ubc.ca: Modified constructor to allow you to set NUM_FRAGMENTS without recompile
  */

RtAudio :: RtAudio(int channels, MY_FLOAT srate, const char *mode, int device, int num_fragments)
{
    HRESULT result;
    BYTE* audioPtr;
    DWORD dataLen;
    ...
    have_good_output_device:
        ...
        */
		...
		ZeroMemory(&directSoundBufferDescription, sizeof(DSBUFFERDESC));
		directSoundBufferDescription.dwSize = sizeof(DSBUFFERDESC);
        // kees changed this: want multiple apps to make sound simultaneously  
		//directSoundBufferDescription.dwFlags = DSBCAPS_STICKYFOCUS | DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_LOCHARDWARE;
        directSoundBufferDescription.dwFlags = DSBCAPS_GLOBALFOCUS | DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_LOCSOFTWARE;
