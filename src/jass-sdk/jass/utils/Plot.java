...
*
*   Copyright (c) April and August 2004, July 2005
*
*   PERMISSION TO COPY:
    	...
    	public void graph(Graphics g){
        	...
        	for(int i=0; i<this.nCurves; i++){
            		...
            		double ycs[]= new double[nxpoints];
                		...
                		for(int ii=1; ii<niPoints; ii++){
                     			xnewpoint=xBot+(int)(((ics[ii]-(double)xLow)/xdenom)*xLen);
                    			ynewpoint=yBot-(int)(((cs.interpolate(ics[ii])-(double)yLow)/ydenom)*yLen);
            		...
            		if(lineOpt[i]==-1 || lineOpt[i]==-2){
                		...
                		yoldpoint=yBot-(int)(((ycs[0]-(double)yLow)/ydenom)*yLen);
                		for(int ii=1; ii<niPoints; ii++){
                    			ynewpoint=yBot+(int)(((ics[ii]-(double)yLow)/ydenom)*yLen);
                    			xnewpoint=xBot-(int)(((cs.interpolate(ics[ii])-(double)xLow)/xdenom)*xLen);
                    			btest2=printCheck(trimOpt[i], xoldpoint, xnewpoint, yoldpoint, ynewpoint);
                    			if(btest2){
                        			if(this.lineOpt[i]==2){
                            				dsum++;
                    ...
                    if(lineOpt[i]==4){
                		...
                		nlpointsold = 0;
                		nlpointsnew = 0;
                		xdashed[0] = xBot+(int)((((this.data[kk][0])-(double)xLow)/xdenom)*xLen);
                		ydashed[0] = yBot-(int)((((this.data[kk+1][0])-(double)yLow)/ydenom)*yLen);
                		for(int ii=1; ii<nxpoints; ii++){
                		    nlpointsnew = lengths[ii-1]/incrmt;
                		    xdashed[nlpointsnew + nlpointsold] = xBot+(int)((((this.data[kk][ii])-(double)xLow)/xdenom)*xLen);
                    		ydashed[nlpointsnew + nlpointsold] = yBot-(int)((((this.data[kk+1][ii])-(double)yLow)/ydenom)*yLen);
